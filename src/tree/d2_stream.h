#ifndef CRYPTANALYSISLIB_TREE_D2_STREAM_H
#define CRYPTANALYSISLIB_TREE_D2_STREAM_H
...
template<const uint32_t k_lower1, const uint32_t k_upper1,
    ...
    requires HashMapAble<HashMap1> &&
             HashMapAble<HashMap2>
#endif
size_t Tree_T<List, config>::twolevel_streamjoin_on_iT_hashmap_v2(
											ExecPolicy &&policy,
											List &out,
                                            const HashMap1 &hmiL,
                                            const List &L1,
	...
	if (is_seq<ExecPolicy>(policy) || nthreads == 0) {
		return twolevel_streamjoin_on_iT_hashmap_v2
				<k_lower1, k_upper1, k_lower2, k_upper2, HashMap1, HashMap2>
				(out, L1, L2, hmiL, hmL2, target, iT, f);
	}
	...
	for (size_t tid = 0; tid < nthreads; tid++) {
		...
		futures.emplace_back(task_pool.enqueue(
				[tid, &out, &hmiL, &L1, &hmL2, &L2, &target, &iT, &f]
				() __attribute__((always_inline)) {

			/// TODO diesen teil der function wird irgendwie hundert mal copy&pasted
