...
namespace cryptographic::atomic
{
using namespace std::string_literals;

std::string acquire_result_acquired          = "acquired"s;
std::string acquire_result_semaphore_stopped = "semaphore_stopped"s;
std::string acquire_result_unknown           = "unknown"s;

class semaphore {semaphore(const semaphore&) = delete;
...
acquire_operation::acquire_operation(semaphore& s) : m_semaphore(s)
{
}

auto acquire_operation::await_ready() const noexcept -> bool
{
...
auto acquire_operation::await_suspend(std::coroutine_handle<> awaiting_coroutine) noexcept -> bool
{
...
auto acquire_operation::await_resume() const -> acquire_result
{
    if (m_semaphore.m_notify_all_set.load(std::memory_order::relaxed))
    {
