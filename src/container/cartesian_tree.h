#pragma once

#include <utility>

class cartesian_tree {
	struct node {
		int x, y, sz;
		node *l, *r;
		node(int _x, int _y) : x(_x), y(_y), sz(1), l(nullptr), r(nullptr) {}
	};

	int tsize(node *t) {
		return t ? t->sz : 0;
	}

	void augment(node *t) {
		t->sz = 1 + tsize(t->l) + tsize(t->r);
	}

    std::pair<node *, node *> split(node *t, int x) {
		if (!t) { return std::make_pair((node *)nullptr, (node *)nullptr); }
		if (t->x < x) {
            std::pair<node *, node *> res = split(t->r, x);
			t->r = res.first;
			augment(t);
			return std::make_pair(t, res.second);
		}
        std::pair<node *, node *> res = split(t->l, x);
		t->l = res.second;
		augment(t);
		return std::make_pair(res.first, t);
	}

	node *merge(node *l,
                node *r) noexcept {
		if (!l) return r;
		if (!r) return l;
		if (l->y > r->y) {
			l->r = merge(l->r, r);
			augment(l);
			return l;
		}
		r->l = merge(l, r->l);
		augment(r);
		return r;
	}

	node *find(node *t, 
               const int x) noexcept {
		while (t) {
			if (x < t->x) t = t->l;
			else if (t->x < x)
				t = t->r;
			else
				return t;
		}

		return nullptr;
	}

	node *insert(node *t, int x, int y) noexcept {
		if (find(t, x) != nullptr) return t;
        std::pair<node *, node *> res = split(t, x);
		return merge(res.first, merge(new node(x, y), res.second));
	}

	node *erase(node *t, int x) noexcept {
		if (!t) return nullptr;
		if (t->x < x) t->r = erase(t->r, x);
		else if (x < t->x)
			t->l = erase(t->l, x);
		else {
			node *old = t;
			t = merge(t->l, t->r);
			delete old;
		}
		if (t) augment(t);
		return t;
	}
	int kth(node *t, int k) {
		if (k < tsize(t->l)) return kth(t->l, k);
		else if (k == tsize(t->l))
			return t->x;
		else
			return kth(t->r, k - tsize(t->l) - 1);
	}
};
