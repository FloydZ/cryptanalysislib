...
public:
	...
	/// TODO maybe two bit mutex is better?
...
struct ConcurrentQueueConfig{
	...
	static const size_t MAX_SUBQUEUE_SIZE = std::numeric_limits<index_t>::max();
...
// TODO in eine seperate datei auslagen und überall nutzen.
// 	andere container solten die auch benutzen für bulk operations
//  verstehene
...
// TODO implement
/// simplified version of: https://github.com/cameron314/concurrentqueue/blob/master/concurrentqueue.h
/// \tparam T
/// \tparam Traits
...
//				}
//
//				if constexpr (!noexcept_CTOR(T, U, new (static_cast<T*>(nullptr)) T(std::forward<U>(element)))) {
...
//					// Dequeue
//					auto& el = *((*block)[index]);
//					if (!noexcept_ASSIGN(T, T&&, element = std::move(el))) {
...
//				}
//
//				if constexpr (noexcept_CTOR(T, decltype(*itemFirst), new (static_cast<T*>(nullptr)) T(details::deref_noexcept(itemFirst)))) {
...
//							// be executed
//							new ((*this->tailBlock)[currentTailIndex]) T(details::nomove_if<!noexcept_CTOR(T, decltype(*itemFirst), new (static_cast<T*>(nullptr)) T(details::deref_noexcept(itemFirst)))>::eval(*itemFirst));
//							++currentTailIndex;
//							++itemFirst;
//						}
//					}
...
//						if (noexcept_ASSIGN(T, T&&, details::deref_noexcept(itemFirst) = std::move((*(*block)[index])))) {
...
//	friend void moodycamel::swap(typename ConcurrentQueue<XT, XTraits>::ImplicitProducerKVP&, typename ConcurrentQueue<XT, XTraits>::ImplicitProducerKVP&) noexcept;
//
