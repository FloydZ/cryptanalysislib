...
public:
	...
	/// \return this
	kAryContainer_T& operator =(kAryContainer_T const &obj) {
	...
	const auto end() const { return __data.end();}

	// this data container is never binary
	__FORCEINLINE__ constexpr static bool binary() { return false; }
	__FORCEINLINE__ constexpr static uint32_t size() { return length; }
...
//#endif
...
#ifdef USE_AVX2
	static inline __m256i neg_mod3_limb256(const __m256i a) {
	...
	static inline T mod3_limb_withoutcorrection(const T a) {
	...
	static inline __uint128_t mod3_limb128(const __uint128_t a) {
	...
	static inline TT add_mod3_limb_no_overflow(const TT a, const TT b) {
	...
	static inline __uint128_t add_mod3_limb128(const __uint128_t x, const __uint128_t y) {
		//int(0b1010101010101010101010101010101010101010101010101010101010101010)
		constexpr __uint128_t c2  = __uint128_t(12297829382473034410u) << 64 | 12297829382473034410u;
		// int(0b0101010101010101010101010101010101010101010101010101010101010101)
		constexpr __uint128_t c1  = __uint128_t(6148914691236517205u) << 64 | 6148914691236517205u;
...
#ifdef USE_AVX2
	static inline __m256i add_mod3_limb_no_overflow(const __m256i a, const __m256i b) {
		// no overflow will happen.
		return _mm256_add_epi64(a,b);
	}


	static inline __m256i add_mod3_limb256(const __m256i x, const __m256i y) {
		const static __m256i c1 = _mm256_set_epi64x(6148914691236517205u,6148914691236517205u,6148914691236517205u,6148914691236517205u);
		const static __m256i c2 = _mm256_set_epi64x(12297829382473034410u,12297829382473034410u,12297829382473034410u,12297829382473034410u);
	...
	static inline T times2_mod3_limb(const T a) {
		// int(0b0101010101010101010101010101010101010101010101010101010101010101)
	...
	static inline bool filter2_mod3_limb(const T a) {
		// int(0b1010101010101010101010101010101010101010101010101010101010101010)
	...
	static inline uint32_t filter2count_mod3_limb(const T a) {
		// int(0b1010101010101010101010101010101010101010101010101010101010101010)
		constexpr T m = T(12297829382473034410u);
		return __builtin_popcountll(a&m);
	}

	static inline uint32_t filter2count_mod3_limb128(const __uint128_t a) {
	...
	static inline uint32_t filter2count_range_mod3_limb(const T a) {
	...
	///			false otherwise
	inline bool filter2_mod3(const uint32_t limit) const {
		uint32_t ctr = 0;
		for (uint32_t i = 0; i < internal_limbs; ++i) {
	...
	inline static void times2_mod3(kAryPackedContainer_T &v3, kAryPackedContainer_T &v1) {
		uint32_t i = 0;
		for (; i < internal_limbs; i++){
	...
	inline static bool add(kAryPackedContainer_T &v3, kAryPackedContainer_T const &v1, kAryPackedContainer_T const &v2,
	                       const uint32_t k_lower, const uint32_t k_upper, const uint32_t filter2) {
		ASSERT(k_upper <= length && k_lower < k_upper);
		for (unsigned int i = k_lower; i < k_upper; i++) {
			DataType data = v1.get(i) + v2.get(i);
			v3.set(data % MOD, i);
	...
	bool is_equal(kAryPackedContainer_T const &obj) const {
		return cmp(*this, obj, 0, length);
	}

	bool is_equal(kAryPackedContainer_T const &obj, const uint32_t k_lower, const uint32_t k_upper) const {
		return cmp(*this, obj, k_lower, k_upper);
	}

	bool is_greater(kAryPackedContainer_T const &obj, const uint32_t k_lower, const uint32_t k_upper) const {
		ASSERT(k_upper <= length && k_lower < k_upper);
	...
	bool is_lower(kAryPackedContainer_T const &obj, const uint32_t k_lower, const uint32_t k_upper) const {
		ASSERT(k_upper <= length && k_lower < k_upper);
		for (unsigned int i = k_lower; i < k_upper; i++) {
			if(get(i) > obj.get(i))
				return false;
	...
	static uint16_t add_only_upper_weight_partly(kAryPackedContainer_T &v3,
	                                             ...
	                                             const uint32_t l, const uint32_t h) {
		uint16_t weight = 0;
		add(v3, v1, v2);
	...
	void left_shift(const uint32_t i) {
	...
	DataType operator [](uint64_t i) {
		ASSERT(i < length && "wrong access index");
		return get(i);
	}

	DataType operator [](const uint64_t i) const {
		ASSERT(i < length && "wrong access index");
		return get(i);
	};

	kAryPackedContainer_T& operator =(kAryPackedContainer_T const &obj) {
		ASSERT(size() == obj.size() && "äh?");
...
#ifdef USE_AVX2
	static void print(const __m256i t) {
	...
	// print the `pos
	void static print_binary(const uint64_t data, const uint16_t k_lower, const uint16_t k_higher) {
		uint64_t d = data;
	...
	// print the `pos
	void print_binary(const uint16_t k_lower, const uint16_t k_higher) const {
		for (int i = k_lower; i < k_higher; ++i) {
	...
	__FORCEINLINE__ constexpr static bool binary() { return false; }
	__FORCEINLINE__ constexpr static uint32_t size() { return length; }
	__FORCEINLINE__ constexpr static uint32_t limbs() { return internal_limbs; }
	__FORCEINLINE__ constexpr static uint32_t bytes() { return internal_limbs*sizeof(T); }

	std::array<T, internal_limbs>& data() { return __data; }
	const std::array<T, internal_limbs>& data() const { return __data; }
	//T& data(const size_t index) { ASSERT(index < length && "wrong index"); return __data[index]; }
	const T data(const size_t index) const { ASSERT(index < length && "wrong index"); return __data[index]; }
	T limb(const size_t index) { ASSERT(index < length && "wrong index"); return __data[index]; }
	const T limb(const size_t index) const { ASSERT(index < length && "wrong index"); return __data[index]; }

	// get raw access to the underlying data.
	T* ptr() { return __data.data(); }
	const T* ptr() const { return __data.data(); }
...
private:
	...
	constexpr static uint64_t popcount(LimbType x, uint64_t mask) {
		if constexpr(limb_bits_width() == 64) {
...
public:
	...
	constexpr static uint16_t limb_bytes_width() { return sizeof(LimbType); };

private:
	// DO NOT CALL THIS FUNCTION. Use 'limbs()'.
	constexpr static uint16_t compute_limbs() {
...
public:
	...
	/// default constructor
	BinaryContainer(): __data() { ASSERT(length > 0); }
	...
	constexpr static LimbType mask(uint16_t i) {
		ASSERT(i <= length && "wrong access index");
	...
	constexpr static LimbType lower_mask2(const uint16_t i) {
		ASSERT(i <= length);
		LimbType u = i%limb_bits_width();
	...
	constexpr static LimbType lower_mask(const uint16_t i) {
		ASSERT(i <= length);
	...
	constexpr static LimbType higher_mask(const uint16_t i) {
		ASSERT(i <= length);
		return (~((LimbType(1) << (i%limb_bits_width())) - 1));
	}
	...
	// realigned to limb_bits_width().
	constexpr static LimbType lower_mask_inverse(const uint16_t i)
	{
		ASSERT(i <= length && "wrong access index");
		LimbType u = i%limb_bits_width();
	...
	constexpr static LimbType higher_mask_inverse(const uint16_t i) {
		ASSERT(i <= length && "wrong access index");
		return (~lower_mask_inverse(i));
	}

	// not shifted.
	constexpr LimbType get_bit(const uint16_t i) const {
		return __data[round_down_to_limb(i)] & mask(i);
	}

	// shifted.
	constexpr bool get_bit_shifted(const uint16_t i) const {
		return (__data[round_down_to_limb(i)] & mask(i)) >> i;
	}

	// return the bits [i,..., j) in one limb
	inline LimbType get_bits(const uint64_t i, const uint64_t j) const {
...
#define __WRITE_BIT(w, spot, value) ((w) = (((w) & ~(m4ri_one << (spot))) | (__M4RI_CONVERT_TO_WORD(value) << (spot))))
	...
	constexpr void zero(const uint64_t k_lower, const uint64_t k_upper) {
	...
	void one(const uint64_t k_lower, const uint64_t k_upper) {
		const uint64_t lower = round_down_to_limb(k_lower);
		const uint64_t upper = round_down_to_limb(k_upper);
	...
	LimbType static random_limb(const uint64_t k_lower, const uint64_t k_upper) {
	...
	void get_bits_set(uint32_t *P, const uint16_t pos=1) {
		uint16_t ctr = 0;
		for (uint32_t i = 0; i < length; ++i) {
			if (get_bit(i)) {
				P[ctr++] = i;
	...
	void column_from_m4ri(const mzd_t *H, const uint32_t col, const uint32_t srow){
		ASSERT(uint64_t(H->ncols) > uint64_t(col));
		for (int i = srow; i < H->nrows; ++i) {
			write_bit(i-srow, mzd_read_bit(H, i, col));
		}
	}
	void from_m4ri(const word *a){
		for (uint32_t i = 0; i < limbs(); ++i) {
			__data[i] = a[i];
		}
	}
	void to_m4ri(mzd_t *a){
		ASSERT(a->nrows == 1 && a->ncols > 0);
		to_m4ri(a->rows[0]);
	}
	void from_m4ri(const mzd_t *a){
		ASSERT(a->nrows == 1 && a->ncols > 0);
		from_m4ri(a->rows[0]);
	}
	...
	void flip(const uint16_t i) {
		ASSERT(i < length);
		__data[round_down_to_limb(i)] ^= mask(i);
	}

	inline void neg(const uint64_t k_lower, const uint64_t k_upper) {
		// do nothing.
	}
	...
	// windowed addition.
	inline void add(BinaryContainer const &v, const uint64_t k_lower, const uint64_t k_upper) {
	...
	static void add_withoutasm(BinaryContainer &v3, BinaryContainer const &v1, BinaryContainer const &v2) {
		for (uint32_t i = 0; i < limbs(); ++i) {
			v3.__data[i] = v1.__data[i] ^ v2.__data[i];
		}
	}
	...
	__FORCEINLINE__ static void add(BinaryContainer &v3, BinaryContainer const &v1, BinaryContainer const &v2) {
...
//		}
	...
	static void add(BinaryContainer &v3, BinaryContainer const &v1, BinaryContainer const &v2) {
	...
	inline static uint32_t add_only_upper_weight_partly(BinaryContainer &v3, BinaryContainer const &v1, BinaryContainer const &v2) {
		int32_t i = 0;
		uint32_t hm = 0;
...
// AVX optimisation disable, because i dont know a good way to calculate the hamming weight of one
	...
	template<const uint32_t ulimb,const LimbType rmask>
	inline static uint32_t add_only_upper_weight_partly_withoutasm(BinaryContainer &v3, BinaryContainer const &v1, BinaryContainer const &v2) {
		uint32_t i = 0;
		uint32_t hm = 0;
	...
	static uint32_t add_only_upper_weight_partly_withoutasm_earlyexit(BinaryContainer &v3, BinaryContainer const &v1, BinaryContainer const &v2) {
		uint32_t hm = 0;
	...
	// TODO optimize
	template<const uint32_t ulimb,const LimbType rmask, const uint32_t early_exit>
	static uint32_t add_only_upper_weight_partly_earlyexit(BinaryContainer &v3, BinaryContainer const &v1, BinaryContainer const &v2) {
		uint32_t hm = 0;
		uint32_t i = 0;
#ifdef USE_AVX2
#ifdef USE_AVX2_SPECIAL_ALIGNMENT
	...
	template<const uint32_t ulimb,const LimbType rmask>
	static uint32_t add_only_upper_weight_partly(LimbType *v3, LimbType const *v1, LimbType const *v2) {
		int32_t i = 0;
		uint32_t hm = 0;
...
//#endif
	...
	/// \tparam rmask	mask which cancel out unwanted bits on the last
    template<const uint32_t ulimb, const LimbType rmask>
    constexpr static void add_only_upper(LimbType *v3, LimbType const *v1, LimbType const *v2) {
	...
	inline constexpr static uint32_t add_weight(LimbType *v3, LimbType const *v1, LimbType const *v2) {
		uint32_t cnorm = 0;
		for (uint32_t i = 0; i < limbs(); ++i) {
			v3[i] = v1[i] ^ v2[i];
			cnorm += popcount(v3[i]);
	...
	inline constexpr static uint32_t add_weight(BinaryContainer &v3, BinaryContainer const &v1, BinaryContainer const &v2) {
		uint32_t cnorm = 0;
		for (uint32_t i = 0; i < limbs(); ++i) {
			v3.__data[i] = v1.__data[i] ^ v2.__data[i];
			cnorm += popcount(v3.__data[i]);
	...
	/// alias for add
	inline void sub(BinaryContainer const &v, const uint64_t k_lower, const uint64_t k_upper) {
	...
	inline constexpr static bool cmp_ext2(BinaryContainer const &v1, BinaryContainer const &v2,
	                           const uint32_t lower, const uint32_t upper, const LimbType lmask, const LimbType umask) {
	...
	inline constexpr static bool cmp_simple2(BinaryContainer const &v1, BinaryContainer const &v2, const uint64_t limb, const LimbType mask) {
		ASSERT(limb != uint64_t(-1) && mask != 0);
		return ((v1.__data[limb]&mask) == (v2.__data[limb]&mask));
	}

	inline constexpr static int cmp_ternary_simple2(BinaryContainer const &v1, BinaryContainer const &v2, const uint64_t limb, const LimbType mask) {
	...
	inline constexpr static void set(BinaryContainer &v1, BinaryContainer const &v2, const uint64_t k_lower, const uint64_t k_upper) {
		ASSERT(k_upper <= length && k_lower < k_upper);
	...
	///  out[s: ] = in[0:s]
	static inline void shift_right(BinaryContainer &out, const BinaryContainer &in, const uint64_t s) {
		for (int j = 0; j < s; ++j) {
	...
	/// \return
	inline bool is_equal(const BinaryContainer &obj, const uint32_t k_lower, const uint32_t k_upper) const {
		return cmp(*this, obj, k_lower, k_upper);
	}

	template<const uint32_t k_lower, const uint32_t k_upper>
	inline bool is_equal(const BinaryContainer &obj) const {
	...
	inline bool is_equal_simple2(const BinaryContainer &obj, const uint64_t limb, const LimbType mask) const {
		return cmp_simple2(*this, obj, limb, mask);
	}

	template<const uint32_t k_lower, const uint32_t k_upper>
	inline bool is_greater(BinaryContainer const &obj) const {
		ASSERT(k_upper <= length && k_lower < k_upper);
		constexpr uint32_t lower = round_down_to_limb(k_lower);
		constexpr uint32_t upper = round_down_to_limb(k_upper-1);
		constexpr LimbType lmask = higher_mask(k_lower);
	...
	inline bool is_greater(BinaryContainer const &obj, const uint32_t k_lower, const uint32_t k_upper) const {
		ASSERT(k_upper <= length && k_lower < k_upper);
	...
	inline bool is_greater_simple2(BinaryContainer const &obj, const uint32_t limb, const LimbType mask) const {
		ASSERT(limb < limbs() && mask != 0);
		return ((__data[limb]&mask) > (obj.__data[limb]&mask));
	}

	/// not testet
	inline bool is_greater_equal_simple2(BinaryContainer const &obj, const uint32_t limb, const LimbType mask) const {
		ASSERT(limb < limbs() && mask != 0);
		return ((__data[limb]&mask) >= (obj.__data[limb]&mask));
	}
	...
	inline bool is_lower(BinaryContainer const &obj, const uint32_t k_lower, const uint32_t k_upper) const {
		ASSERT(k_upper <= length && k_lower < k_upper);
		int64_t lower = round_down_to_limb(k_lower);
		int64_t upper = round_down_to_limb(k_upper-1);
		const LimbType lmask = higher_mask(k_lower);
	...
	inline bool is_lower_simple2(BinaryContainer const &obj, const uint32_t limb, const LimbType mask) const {
		ASSERT(limb < limbs() < length && mask != 0);
		return ((__data[limb]&mask) < (obj.__data[limb]&mask));
	}

	inline bool is_lower_equal_simple2(BinaryContainer const &obj, const uint32_t limb, const LimbType mask) const {
		ASSERT(limb < limbs() < length && mask != 0);
		return ((__data[limb]&mask) <= (obj.__data[limb]&mask));
	}
	...
	template<const uint32_t ulimb,const LimbType rmask, const uint32_t early_exit>
	inline uint32_t weight_earlyexit() {
		uint32_t hm = 0;
	...
	uint32_t weight(const uint32_t k_lower, const uint32_t k_upper) const {
		ASSERT(k_upper <= length && k_lower < k_upper);
	...
	constexpr static uint32_t weight_sum_only_upper(LimbType *a, LimbType *b) {
		uint32_t weight = 0;
	...
	// length operators
	__FORCEINLINE__ constexpr static bool binary() { return true; }
	__FORCEINLINE__ constexpr static uint32_t size() { return length; }
	__FORCEINLINE__ constexpr static uint32_t limbs() { return (length+limb_bits_width()-1)/limb_bits_width(); }
	__FORCEINLINE__ constexpr static uint32_t bytes() {
#ifdef BINARY_CONTAINER_ALIGNMENT
		return alignment()/8;
#else
	...
	__FORCEINLINE__ LimbType* ptr() { return __data.data(); };
	const __FORCEINLINE__ LimbType* ptr() const { return __data.data(); };

	__FORCEINLINE__ std::array<LimbType, compute_limbs()>& data() { return __data; };
	__FORCEINLINE__ const std::array<LimbType, compute_limbs()>& data() const { return __data; };
