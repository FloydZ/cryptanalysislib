...
//
// TODO:
// 	- bugfixxes as in the github issues
// 	- constexpr 
...
private:
	...
	constexpr static int matchstar(regex &p,
	                               regex *pattern,
	                               const char *text,
	                               int *matchlength) noexcept {
	...
	/* Iterative matching */
	static int matchpattern(regex *pattern, const char *text, int *matchlength) {
		int pre = *matchlength;
		do {
...
public:
	/* Public functions: */
	static int re_match(const char *pattern, const char *text, int *matchlength) {
		const auto kek = re_compile(pattern);
		return re_matchp(kek, text, matchlength);
	}


	static int re_matchp(regex *pattern, const char *text, int *matchlength) {
		*matchlength = 0;
		if (pattern != 0) {
	...
	static regex* re_compile(const char *pattern) {
...
*/
	...
	static void re_print(regex *pattern) noexcept {
		const char *types[] = {"UNUSED", "DOT", "BEGIN", "END", "QUESTIONMARK",
			"STAR", "PLUS", "CHAR", "CHAR_CLASS", "INV_CHAR_CLASS", "DIGIT", 
