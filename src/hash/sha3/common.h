#ifndef CRYPTANALYSISLIB_HASH_SHA3_COMMON_H
#define CRYPTANALYSISLIB_HASH_SHA3_COMMON_H
...
namespace cryptanalysislib::hash {
	...
	template <typename T> concept byte_like =
	        (sizeof(T) == 1u) &&
	        (std::same_as<T, char> ||
	...
	template <typename T, typename Y> concept castable_to =
		requires(T val) {
	        {static_cast<Y>(val)} -> std::same_as<Y>;
	    };
	template <size_t N> struct keccak_suffix {
		...
		constexpr keccak_suffix(unsigned b, castable_to<std::byte> auto... v) noexcept :
		    bits{b}, values{ static_cast<std::byte>(v)...} { }
	};
