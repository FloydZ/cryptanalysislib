...
namespace cryptanalysislib {
    namespace detail {
        ...
        int type_idx(std::type_info const* ti, int size) {
        	...
        	_access[next_typeidx].counter = new std::atomic<std::uint64_t>[size];
        	for (int i = 0; i < size; ++i) _access[next_typeidx].counter[i] = 0;
        	_access[next_typeidx].size = size;
        ...
        void segv_handler(int, siginfo_t *info, void* uap) {
        	...
        	if ((char*)info->si_addr < (char*)base_ptr + type_size)
        	{
        		...
        		++_access[type_idx].counter[offset];
        	}
    ...
    static struct static_init_t {
    	static_init_t() noexcept {
            detail::init_instrumentation();
    	}
