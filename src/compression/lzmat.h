...
int lzmat_encode(uint8_t *pbOut, uint32_t *pcbOut,
                 const uint8_t *pbIn, const size_t cbIn,
                 void *pVocabularyBuf = nullptr) noexcept {
	...
	while (cbIn > inPtr) {
		...
		if ((cbIn - inPtr) >= 4 && *(uint32_t *) pInp == *(uint32_t *) pITmp) {
			...
			while (in_Reminder-- && *pCurPtr++ == *pITmp++) {
                // TODO use this for avx2  optimization 
                ...
                //    
                //    how to count only the first consecutove equal elements
			...
			while (dict_ptr < cur_idx) {
				...
				if (*(uint16_t *) (pITmp + dict_ptr) == cmp_val) {
					...
					if (MP_CMP_DISTANCE(inPtr, Gamma_dist, new_dist, match_cnt, match_found)) {
						...
						pITmp = (uint8_t *)pbIn + match_found - 1;
...
#undef cbCompressed
	...
	*pcbOut = ((size_t) pOut - (size_t) pbOut) + cur_nib;
	return 0;
}
...
#ifdef mkstub
int __fastcall lzmat_decode(uint8_t *pbOut, uint8_t *pbIn, uint32_t cbIn)
#else //!mkstub
int LZMAT_CALLCONV lzmat_decode(uint8_t *pbOut, uint32_t *pcbOut,
	uint8_t *pbIn, uint32_t cbIn)
#endif
{
	uint32_t inPos, outPos;
				...
				else
				{
					...
					else
					{
						...
						if(r_cnt==LZMAT_2BYTE_CNT)
						{
							...
							{
								r_cnt = (GET_LE16(pbIn+inPos-5)&0xFC0)<<1;
								//((uint32_t)(pbIn[inPos-5]&0xC0)+(pbIn[inPos-4]<<4))<<1;
							}
							r_cnt+=(tag&0x7F)+4;
							r_cnt<<=1;
#ifdef mkstub
