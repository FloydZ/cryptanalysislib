...
#define LZMAT_DEFAULT_CNT	(0x12)
#define LZMAT_1BYTE_CNT		(0xFF + LZMAT_DEFAULT_CNT) // 0x111
#define LZMAT_2BYTE_CNT		(0xFFFF + LZMAT_1BYTE_CNT) // 0x10110
#define LZMAT_MAX_2BYTE_CNT	(LZMAT_2BYTE_CNT-1)
...
/// \param pbOut[out];
/// \param pbOut[in/out]: input: max length of the output buffer 
///                       output: actually used buffer size
///
constexpr static 
int lzmat_encode(uint8_t *pbOut, 
                 uint32_t *pcbOut,
                 uint8_t *pbIn, 
                 const uint32_t cbIn) noexcept {
    // min size
    assert(*pcbOut > 0x21);
    ...
    // pointer to the end of the out buffer
	const uint8_t *pEndOut = pbOut + *pcbOut - 0x21;
	...
	*pbOut = *pbIn;
	...
	while(cbIn > inPtr) {
		...
		if( (cbIn-inPtr)>=4 && (*(uint32_t *)pInp == *(uint32_t *)pITmp)) {
            ...
            // limit the number of bytes-matches
