...
static void BM_avx2_sort_nn_on_double32_0(benchmark::State& state) {
	for (auto _ : state) {
		const uint64_t z = fastrandombytes_uint64();
		size_t ne1=0, ne2=0;
...
static void BM_avx2_sort_nn_on_double32_1(benchmark::State& state) {
	for (auto _ : state) {
		const uint64_t z = fastrandombytes_uint64();
		size_t ne1=0, ne2=0;
...
BENCHMARK(BM_avx2_sort_nn_on_double32_0)->RangeMultiplier(2)->Range(1024, 1u<<18)->Complexity();
BENCHMARK(BM_avx2_sort_nn_on_double32_1)->RangeMultiplier(2)->Range(1024, 1u<<18)->Complexity();
