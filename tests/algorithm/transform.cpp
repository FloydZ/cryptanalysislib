...
//TEST(transform, multithreaded_single) {
//    constexpr static size_t s = 100;
//    using T = int;
//    std::vector<T> in; in.resize(s);
//    std::vector<T> out; out.resize(s);
...
//
//
//    cryptanalysislib::transform(par_if(false), in.begin(), in.end(), out.begin());
//
//    for (size_t i = 0; i < s; i++) {
//        EXPECT_EQ(in[i], out[i]);
//    }
...
//
//
//    cryptanalysislib::transform(par_if(true), in.begin(), in.end(), out.begin());
//
//    for (size_t i = 0; i < s; i++) {
//        EXPECT_EQ(in[i], out[i]);
//    }
