...
constexpr static uint64_t SIZE = 1<<3;
...
constexpr static uint64_t solution_index = 7;
...
TEST(lower_bound_monobound_interpolated_search, simple) {
	std::vector<ContainerT> data;
	ContainerT search = random_data(data);
	...
	auto a = lower_bound_monobound_interpolated_search(data.begin(), data.end(), search,
        [](const ContainerT &e1) -> ContainerT {
            return e1;
        }
	);
...
//
//
//TEST(lower_bound_adaptive_binary_search, simple) {
//	std::vector<ContainerT> data;
//	ContainerT search = random_data(data);
//
//	auto a = lower_bound_adaptive_binary_search(data.begin(), data.end(), search,
//	                                   [](const ContainerT &e1, const ContainerT &e2) -> bool {
//		                                   return e1 < e2;
//	                                   }
//	);
