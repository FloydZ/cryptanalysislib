...
static const __uint128_t kSeed128 = (__uint128_t(kSeed0)) ^ (__uint128_t(kSeed1) << 64u);
...
using namespace cryptanalysislib::hash;
...
TEST(cityhash, all) {
	...
	int i = 0;
	for ( ; i < kTestSize - 1; i++) {
		InternalTest(testdata[i], i * i, i);
	}
	InternalTest(testdata[i], 0, kDataSize);
}
